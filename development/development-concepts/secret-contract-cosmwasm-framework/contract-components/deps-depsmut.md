...
* **(New in Secret-Cosmwasm v1.1.10)**  `gas_evaporate(evaporate: u32) -> u32` - this API function allows a contract to consume a set amount of gas. Use together with `check_gas` in order to create contract calls that consume an exact amount of gas regardless of the code path taken. [Documentation is here.](../../secret-contract-fundamentals/privacy-design/gas-evaporation-and-tracking.md)
